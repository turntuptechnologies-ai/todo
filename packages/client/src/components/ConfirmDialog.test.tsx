...
describe('ConfirmDialog', () => {
  it('メッセージが表示される', () => {
    render(<ConfirmDialog message="本当に削除しますか？" onConfirm={vi.fn()} onCancel={vi.fn()} />);
    expect(screen.getByText('本当に削除しますか？')).toBeInTheDocument();
  });

  it('マウント時に showModal が呼ばれる', () => {
    render(<ConfirmDialog message="確認" onConfirm={vi.fn()} onCancel={vi.fn()} />);
  ...
  it('確認ボタンをクリックすると onConfirm が呼ばれる', async () => {
    ...
    render(<ConfirmDialog message="削除しますか？" onConfirm={onConfirm} onCancel={vi.fn()} />);
